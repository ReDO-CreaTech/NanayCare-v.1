...
function getFlowForPatient() {
  if (!Array.isArray(window.infantFlow) || !Array.isArray(window.childFlow)) {
    console.error("❌ Flow definitions missing");
    return null;
  }
...
function next() {
  ...
  while (step < flow.length && safety < 100) {
    safety++;
...
async function result() {
  ...
  try {
    ...
    if (typeof fn === "function") {
      ...
      console.error("❌ Classification functions missing");
    }
  } catch (e) {
  ...
  const record = window.createPatient({
    ...patient,
    location: coords,
  ...
  await window.savePatient(record);
...
window.showPatientList = async function () {
  const patients = await window.getAllPatients();
...
window.viewPatient = async function (id) {
  const p = await window.getPatientById(id);
